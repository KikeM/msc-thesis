...
for idx, nt in tqdm(enumerate(nts, start=0), total=len(nts)):

    # try:
    solver.BDF_SCHEME = BDF_SCHEME
    solver.setup()
    solver.domain[Domain.NT] = int(nt)
    solver.update_parametrization(mu)
    solver.solve()

    name_probes = f"probes_FOM_{idx}_bdf_{BDF_SCHEME}"
    name_mass = f"mass_FOM_{idx}_bdf_{BDF_SCHEME}"
    probes = solver.save_probes(name=name_probes + ".csv")
    mass = solver.save_mass_conservation(name=name_mass + ".csv")

    plot_probes(probes, save=name_probes)
    plot_mass_conservation(
        ts=mass[MassConservation.TIMESTEPS],
        mass_change=mass[MassConservation.MASS_CHANGE],
        outflow=mass[MassConservation.OUTFLOW],
        title=f"Mass Conservation - FOM {idx}",
        save=name_mass,
    )
