...
#         ax.legend()
#         title = f"N-ROM = {n_rom}, N-SROM = {n_srom}, N-MDEIM = {n_mdeim}"
#         ax.set_title(title)
#         ax.set_xlabel("t (s)")
#         ax.set_ylabel("$L_2$ Error (FOM vs. ROM)")

#         figname = (
#             f"error_estimation_rom_{n_rom}_srom_{n_srom}_mdeim_{n_mdeim}_{idx_mu}.png"
#         )
...
results = pd.DataFrame(results)
results = results.drop_duplicates(subset=["N", "N-MDEIM"])
results = results.sort_values(by=["N", "N-MDEIM"])
results = results.reset_index(drop=True)
results.to_csv("error_decay_heatmap.csv")
results = results.pivot(index="N", columns="N-MDEIM", values="error")
print(results.to_latex(float_format="%.1e"))
results = results.apply(np.log10)

ax = sns.heatmap(results, vmin=-8, vmax=0, annot=True, linewidths=0.5, cbar=False)
ax.set_title("Timewise $L_2$ Error ($log_{10}$)")
plt.savefig("error_decay_heatmap.png", **FIG_KWARGS)
plt.close()
